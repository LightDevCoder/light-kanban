 ...
 * confirmation, a few archived; 3 agents with uploaded image avatars; 4
 * workspaces; long/short titles; no/long descriptions; single/multi tags;
...
// 1x1 solid-color PNGs (generated via python zlib/struct).
const AVATARS = {
  codex: 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAADElEQVR4nGOoiX0DAAMeAcb2Fcf1AAAAAElFTkSuQmCC',
  claude: 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAADElEQVR4nGOwbvoGAAKvAbS3T615AAAAAElFTkSuQmCC',
  grok: 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAADElEQVR4nGMQmdYDAAH4ATfxYrXcAAAAAElFTkSuQmCC',
}
const AGENTS = [
  { agentId: 'codex', name: 'Codex' },
  { agentId: 'claude', name: 'Claude' },
  { agentId: 'grok', name: 'Grok' },
]
...
async function uploadAvatar(b64) {
  ...
  fd.append('file', new Blob([Buffer.from(b64, 'base64')], { type: 'image/png' }), 'avatar.png')
...
async function main() {
  ...
  for (const a of AGENTS) a.avatar = await uploadAvatar(AVATARS[a.agentId])
  ...
  const plan = [
    ...
    { t: 'Improve workspace detection', ws: WS.kanban, tags: ['workspace'], agent: 'claude' },
    { t: '重写个人站点首页 Hero 区', ws: WS.site, tags: ['ui'], agent: 'codex' },
    { t: 'PAE 知识库数据导入管道', ws: WS.pae, tags: ['data'], agent: 'grok', desc: 'CSV → 清洗 → 入库 → 索引重建，全流程幂等。' },
    ...
    { t: '等待新版首页设计稿确认', ws: WS.site, tags: ['设计'], agent: 'claude', blockReason: '需要用户确认设计稿 v3 的配色与排版' },
    { t: 'Upstream data source rate limited', ws: WS.pae, tags: ['data'], agent: 'grok', blockReason: '数据源限流 (429)，明天 10:00 后重试' },

    // ---- 等你确认 (12) ----
    { t: 'Update bilingual documentation', ws: WS.kanban, tags: ['docs', 'i18n'], agent: 'claude', done: true },
    { t: 'Fix macOS folder picker', ws: WS.kanban, tags: ['macOS', 'ui'], agent: 'codex', done: true },
    { t: 'Add compact task cards', ws: WS.kanban, tags: ['ui', 'card'], agent: 'codex', done: true },
    { t: '统一网页端与 agent 的看板持久化路径', ws: WS.kanban, tags: ['persistence'], agent: 'grok', done: true },
    { t: '详情页支持标签选择与创建', ws: WS.kanban, tags: ['ui', '标签'], agent: 'claude', done: true },
    { t: 'Remove legacy vanilla JS frontend', ws: WS.kanban, tags: ['frontend'], agent: 'codex', done: true },
    { t: '完成 v1.0.1 发布与四平台二进制', ws: WS.kanban, tags: ['release'], agent: 'grok', done: true },
    { t: 'PAE 知识库搜索索引重建', ws: WS.pae, tags: ['search'], agent: 'codex', done: true },
    { t: '个人站点暗色模式', ws: WS.site, tags: ['ui', 'darkmode'], agent: 'claude', done: true },
    { t: 'README 快速上手截图更新', ws: WS.kanban, tags: ['docs'], agent: 'grok', done: true },
    { t: 'Verify Windows double-click launch', ws: WS.kanban, tags: ['windows', 'release'], agent: 'codex', done: true },
    { t: '归档历史支持多选删除', ws: WS.kanban, tags: ['archive'], agent: 'claude', done: true },

    // ---- 已归档 (3) ----
    { t: '搭建 SQLite store 层与状态机', ws: WS.kanban, tags: ['go', 'sqlite'], agent: 'codex', done: true, archived: true },
    { t: '四列看板首版上线', ws: WS.kanban, tags: ['ui'], agent: 'claude', done: true, archived: true },
    { t: 'Agent 自注册与头像约束', ws: WS.kanban, tags: ['agent'], agent: 'grok', done: true, archived: true },
  ]
