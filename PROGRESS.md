...
- **vendored 快照完整性强化**：`scripts/verify-vendored-skill.cjs` 从「只校验 manifest 列出的文件」升级为「实际递归文件集 === manifest 文件集」——缺文件、hash 漂移、**多出来的未登记文件**全部 FAIL；`--self-test` 扩到 8 断言（positive + 改 SKILL.md → hash mismatch + 删 SKILL.md → missing + 新增 unexpected-extra.md → unexpected file，extra-file 负例在全新副本上隔离验证），接入 `make check`/CI
...
- **发布记录**：Skills v0.1.4 先发布并 fresh-install 验证；Light-Kanban 经用户验收授权后发布——tag `v1.0.5`（commit `9cb7e98`）+ GitHub Release（https://github.com/LightDevCoder/light-kanban/releases/tag/v1.0.5），四平台二进制与 `light-kanban-worker.zip` 快照作为 release 资产；`make check` / `make cross` / CI 均 PASS
